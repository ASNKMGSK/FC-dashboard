  ...
  return (
    <div>
      <div className="flex items-end justify-between gap-3 mb-3">
        <div>
          <h2 className="text-lg md:text-xl font-semibold text-sf-brown">사용자 관리</h2>
          <p className="text-sm text-sf-brown/60">계정 목록 확인 및 추가</p>
        </div>
      </div>

      <div className="card mb-4">
        <div className="card-header">사용자 목록</div>
        {loading ? <div className="text-sm text-sf-brown/60">로딩 중...</div> : null}
          ...
          <table className="table">
            ...
            <tbody>
              {users?.length ? users.map((u, idx) => (
                ...
                <tr><td colSpan={VISIBLE_COLUMNS.length} className="py-3 text-sf-brown/70 text-center">데이터 없음</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <div className="card">
        ...
        {msg ? (
          <div className="mt-3 rounded-xl border border-sf-orange/20 bg-sf-beige px-3 py-2 text-sm text-sf-brown/80">
            {msg}
          </div>
        ) : null}
      </div>
